...
def setup(level):
    ...
    number_count = min(number_count, 20) # 만약에 20을 초과하면 20으로 처리
...
def shuffle_grid(number_count):
    ...
    number = 1 # 시작 숫자 1부터 number_count 까지, 만약 5라면 숫자를 랜덤으로 배치
    while number <= number_count:
        row_idx = randrange(0, rows) # 0 , 1, 2, 3,4 중에서 랜덤으로 뽑기
        col_idx = randrange(0, columns) #0~8까지 랜덤으로 뽑기
...
def display_start_screen():
    ...
    # 흰색으로 동그라미르 그리는데 중심좌표는 start_button의 중심좌표를 따라가고,
    # 반지름은 60, 선 두께는 5
...
def display_game_screen():
    ...
    if not hidden:
        elapsed_time = (pygame.time.get_ticks() - start_ticks) /1000 #ms to sec
...
while running:
    ...
    for event in pygame.event.get(): # 어떤 이벤트가 발생했는가?
        if event.type == pygame.QUIT: # 창이 닫히는 이벤트 인가?
            running = False # 게임이 더 이상 실행중이 아님
        elif event.type == pygame.MOUSEBUTTONUP: # 사용자가 마우스를 클릭 했을때
...
pygame.time.delay(5000) # 5초 정도 보여줌
